...
with tempfile.TemporaryDirectory(prefix="homeserver-agent-workflow-automation-v058-") as data_dir:
    ...
    def create_plan(conversation_id: str, source: str = "owner") -> int:
        with db() as connection:
            ...
            connection.execute(
                ...
                (conversation_id, int(primary["id"]), source, conversation_id),
            )
    ...
    # One-time creation is explicit/idempotent and executes exactly once.
    once_plan = create_plan("v058-once")
    once = automation.create_automation(
        "owner", "v058-once", once_plan,
        trigger_type="once", run_at=(base + timedelta(minutes=5)).isoformat(),
        owner=True, now=base,
    )
    ...
    assert duplicate["automation_id"] == once["automation_id"] and duplicate["reused"] is True
    first = runtime.run_due_automations(now=base + timedelta(minutes=6))
    assert first["claimed"] == 1 and first["completed"] == 1 and first["errors"] == 0
    assert runtime.run_due_automations(now=base + timedelta(minutes=6))["claimed"] == 0
    assert len(supervised_calls) == 1
    once_after = automation.list_automations("owner", conversation_id="v058-once")[0]
    assert once_after["enabled"] is False and once_after["last_status"] == "completed"
    assert len(automation.list_automation_runs("owner", once["automation_id"])) == 1

    # Interval triggers coalesce missed slots and do not replay a backlog.
    interval_plan = create_plan("v058-interval")
    interval = automation.create_automation(
        "owner", "v058-interval", interval_plan,
        trigger_type="interval", every_seconds=60,
        run_at=(base + timedelta(minutes=10)).isoformat(), max_steps=1,
        owner=True, now=base,
    )
    tick = base + timedelta(minutes=12, seconds=30)
    interval_run = runtime.run_due_automations(now=tick)
    assert interval_run["claimed"] == 1 and interval_run["completed"] == 1
    interval_after = automation.list_automations("owner", conversation_id="v058-interval")[0]
    assert interval_after["enabled"] is True
    next_run = datetime.fromisoformat(interval_after["next_run_at"])
    assert next_run > tick
    assert runtime.run_due_automations(now=tick)["claimed"] == 0
    assert runtime.run_due_automations(now=next_run + timedelta(seconds=1))["claimed"] == 1
    assert len(automation.list_automation_runs("owner", interval["automation_id"])) == 2
    automation.set_automation_enabled("owner", interval["automation_id"], False, now=tick)

    # Exact activity matching starts at the creation watermark. A nonmatching
    # event is ignored and a matching event is consumed once.
    activity_plan = create_plan("v058-activity")
    activity = automation.create_automation(
        "owner", "v058-activity", activity_plan,
        trigger_type="activity", activity_action="task.reminded",
        resource_type="task", resource_key="42", owner=True, now=base,
    )
    with db() as connection:
        connection.execute(
    ...
    event_run = runtime.run_due_automations(now=base + timedelta(minutes=20))
    assert event_run["claimed"] == 1 and event_run["completed"] == 1
    assert runtime.run_due_automations(now=base + timedelta(minutes=20))["claimed"] == 0
    assert len(automation.list_automation_runs("owner", activity["automation_id"])) == 1
    try:
        automation.create_automation(
            "owner", "v058-activity", activity_plan,
            trigger_type="activity", activity_action="agent.workflow.supervision.stopped",
            owner=True, now=base,
        )
    ...
    # Two scheduler callers cannot claim one trigger twice.
    race_plan = create_plan("v058-race")
    race = automation.create_automation(
        "owner", "v058-race", race_plan,
        trigger_type="once", run_at=(base + timedelta(minutes=30)).isoformat(),
        owner=True, now=base,
    )
    before_race = len(supervised_calls)
    barrier = threading.Barrier(3)
    race_results: list[dict[str, int]] = []

    def worker():
        barrier.wait()
        race_results.append(runtime.run_due_automations(now=base + timedelta(minutes=31)))
    ...
    assert sum(item["claimed"] + item["recovered"] for item in race_results) == 1
    assert len(supervised_calls) == before_race + 1
    assert len(automation.list_automation_runs("owner", race["automation_id"])) == 1

    # Paired-app permissions are live at trigger time. Revoking agent.chat
    # disables the automation without entering v0.57.
    with db() as connection:
        app_cursor = connection.execute(
            "INSERT INTO paired_apps(app_key, name, token_hash, status) VALUES ('demo', 'Demo App', 'synthetic-hash', 'active')"
        )
        app_id = int(app_cursor.lastrowid)
        connection.execute(
    ...
    app_auto = automation.create_automation(
        "app:demo", "v058-app", app_plan,
        trigger_type="once", run_at=(base + timedelta(minutes=40)).isoformat(),
        owner=False, current_permissions={"agent.chat"}, now=base,
    )
    with db() as connection:
        connection.execute(
    ...
    before_revoke = len(supervised_calls)
    revoked = runtime.run_due_automations(now=base + timedelta(minutes=41))
    assert revoked["claimed"] == 1 and revoked["conflicts"] == 1
    assert len(supervised_calls) == before_revoke
    app_after = automation.list_automations("app:demo", conversation_id="v058-app")[0]
    assert app_after["enabled"] is False and app_after["last_status"] == "conflict"

    # Proposed plans remain an explicit owner approval boundary.
    ...
    try:
        automation.create_automation(
            "owner", "v058-proposed", proposed_plan,
            trigger_type="once", run_at=(base + timedelta(hours=1)).isoformat(),
            owner=True, now=base,
        )
        raise AssertionError("proposed workflow should not be automatable")
    except automation.AgentWorkflowAutomationError as exc:
        assert exc.status_code == 409 and "approved" in str(exc).lower()
