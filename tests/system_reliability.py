...
with tempfile.TemporaryDirectory(prefix="homeserver-system-") as data_dir:
    ...
    assert settings.version == "0.11.0"
    ...
    with TestClient(app) as owner:
        assert owner.get("/api/v1/health").json()["version"] == "0.11.0"
        ...
        assert payload["diagnostics"]["version"] == "0.11.0"
        assert payload["diagnostics"]["database"]["ok"] is True
        assert payload["diagnostics"]["database"]["schema_version"] == 9
