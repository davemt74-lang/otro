...
with tempfile.TemporaryDirectory(prefix="homeserver-system-") as data_dir:
    ...
    assert settings.version == "0.13.0"
    ...
    with TestClient(app) as owner:
        assert owner.get("/api/v1/health").json()["version"] == "0.13.0"
        ...
        assert payload["diagnostics"]["version"] == "0.13.0"
        assert payload["diagnostics"]["database"]["ok"] is True
        assert payload["diagnostics"]["database"]["schema_version"] == 11
        ...
        # Source/test launches are not allowed to write the installed Windows
        # startup registration even when CI itself runs on Windows.
