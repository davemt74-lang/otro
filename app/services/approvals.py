...
def _safe_meta(arguments: dict[str, Any]) -> dict[str, Any]:
    return {
        "content_length": len(arguments["content"]),
        "memory_key_length": len(arguments["memory_key"] or ""),
        "importance": arguments["importance"],
    }


def _record_request_tool_run(
    source_app_key: str,
    actor_type: str,
    request_id: str,
...
def create_memory_write_request(
    ...
    *,
    ...
    arguments_meta = _safe_meta(normalized)
    request_id = uuid.uuid4().hex
    expires_at = (_now() + timedelta(hours=24)).isoformat()

    run_id = _record_request_tool_run(source, actor_type, request_id, arguments_meta)
    try:
        with db() as connection:
            connection.execute(
                """
        ...
        with db() as connection:
            connection.execute(
                "UPDATE tool_runs SET status='failed', error='Action request could not be persisted.' WHERE id=?",
                (run_id,),
            )
        raise
...
def approve_request(request_id: str) -> dict[str, Any]:
    ...
    with db() as connection:
        reserved = connection.execute(
            """
            UPDATE action_requests
            SET status='executing', decided_at=CURRENT_TIMESTAMP
...
def deny_request(request_id: str) -> dict[str, Any]:
    ...
    with db() as connection:
        updated = connection.execute(
            """
            UPDATE action_requests
            SET status='denied', decided_at=CURRENT_TIMESTAMP, error=NULL
            WHERE id=? AND status='pending'
            """,
            (request["id"],),
        )
