...
def create_pairing_request(app_key: str, app_name: str, permissions: list[str]) -> dict:
    ...
    with db() as connection:
        connection.execute(
            ...
            """,
            (
                _hash(code), app_key.strip(), app_name.strip(), json.dumps(requested),
                expires.isoformat(), request_id, _hash(claim_token),
            ),
        )
    return {"protocol":"claim-v1","code":code,"request_id":request_id,"claim_token":claim_token,"expires_at":expires.isoformat(),"permissions":requested}
...
def approve_pairing(code: str) -> dict | None:
    with db() as connection:
        request = connection.execute("SELECT * FROM pairing_requests WHERE code_hash=? AND status='pending'", (_hash(code),)).fetchone()
        if request is None or _expire_request(connection, request):
            return None
        claim_hash = request["claim_hash"]
        legacy_token: str | None = None
        if claim_hash:
            token_hash = claim_hash; delivery = "claim_token"
        else:
            legacy_token = secrets.token_urlsafe(48); token_hash = _hash(legacy_token); delivery = "legacy_token"
        connection.execute(
            """INSERT INTO paired_apps(app_key, name, token_hash) VALUES (?, ?, ?)
               ON CONFLICT(app_key) DO UPDATE SET name=excluded.name, token_hash=excluded.token_hash,
        ...
        connection.execute("INSERT OR IGNORE INTO app_capability_scopes(paired_app_id) VALUES (?)", (app["id"],))
        ...
        for permission in permissions:
            connection.execute(
                """INSERT INTO app_permissions(paired_app_id, permission, allowed) VALUES (?, ?, 1)
                   ON CONFLICT(paired_app_id, permission) DO UPDATE SET allowed=1, updated_at=CURRENT_TIMESTAMP""",
                (app["id"], permission),
            )
        ...
        connection.execute(
            ...
            (request["app_key"], json.dumps({"delivery":delivery,"permissions":permissions}, separators=(",", ":"))),
        )
    result = {"app_key":request["app_key"],"permissions":permissions,"delivery":delivery}
...
def pairing_status(request_id: str, claim_token: str) -> dict | None:
    ...
    with db() as connection:
        request = connection.execute("SELECT * FROM pairing_requests WHERE request_id=? AND claim_hash=? LIMIT 1", (request_id.strip(), claim_hash)).fetchone()
        if request is None:
            return None
        ...
        ready = False
        if status == "approved":
            app = connection.execute("SELECT id FROM paired_apps WHERE app_key=? AND token_hash=? AND status='active' LIMIT 1", (request["app_key"], claim_hash)).fetchone()
            ready = app is not None
        return {"protocol":"claim-v1","request_id":request["request_id"],"app_key":request["app_key"],"status":status,"ready":ready,"expires_at":request["expires_at"],"permissions":permissions}


def authenticate(raw_token: str) -> dict | None:
    with db() as connection:
        row = connection.execute("SELECT id, app_key, name FROM paired_apps WHERE token_hash=? AND status='active'", (_hash(raw_token),)).fetchone()
        if row is None:
            return None
    ...
    return {"id":row["id"],"app_key":row["app_key"],"name":row["name"],"permissions":[p["permission"] for p in permissions],"scope":app_scopes.get_scope(int(row["id"]))}
