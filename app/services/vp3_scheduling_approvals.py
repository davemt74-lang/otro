...
def _normalize(action_key:str,arguments:dict[str,Any]|None)->dict[str,Any]:
    ...
    definition=tools.TOOL_DEFINITIONS.get(action_key) or {};schema=definition.get("input_schema") or {};properties=set((schema.get("properties") or {}).keys());required=set(schema.get("required") or [])
    unknown=set(payload)-properties
    if unknown: raise approvals.ApprovalError(f"Unsupported {action_key} proposal argument: {sorted(unknown)[0]}")
    missing=[key for key in required if payload.get(key) in (None,"")]
    if missing: raise approvals.ApprovalError(f"{action_key} proposal requires {missing[0]}.")
    if action_key=="vp3.booking.create":
        kind=str(payload.get("kind") or "");target=int(payload.get("target_id") or 0);name=str(payload.get("guest_name") or "").strip();start=str(payload.get("start_at_utc") or "").strip()
        if kind not in {"personal","team"} or target<1 or not name or len(name)>190 or len(start)<16: raise approvals.ApprovalError("VP3 booking proposal is invalid.")
        payload.update(kind=kind,target_id=target,guest_name=name,start_at_utc=start)
    elif action_key=="vp3.booking.reschedule":
        if int(payload.get("booking_id") or 0)<1 or len(str(payload.get("start_at_utc") or ""))<16: raise approvals.ApprovalError("VP3 reschedule proposal is invalid.")
        payload["booking_id"]=int(payload["booking_id"])
    else:
        kind=str(payload.get("kind") or "");booking=int(payload.get("booking_id") or 0)
        if kind not in {"personal","team"} or booking<1: raise approvals.ApprovalError("VP3 cancellation proposal is invalid.")
        payload.update(kind=kind,booking_id=booking)
    if not str(payload.get("idempotency_key") or "").strip(): payload["idempotency_key"]="hs-action-"+uuid.uuid4().hex
    return payload

def create_request(source_app_key:str,action_key:str,arguments:dict[str,Any]|None,*,owner:bool=False)->dict[str,Any]:
    if action_key not in ACTIONS: raise approvals.ApprovalError("Unsupported VP3 scheduling action.")
    source=source_app_key.strip() or ("owner" if owner else "app:unknown");actor="owner" if owner else "app";required=[] if owner else ["scheduling.write","tools.execute"]
    ...
    meta={"kind":str(normalized.get("kind") or ""),"target_id":int(normalized.get("target_id") or 0),"booking_id":int(normalized.get("booking_id") or 0),"guest_name_present":bool(normalized.get("guest_name")),"guest_email_present":bool(normalized.get("guest_email"))}
    return approvals._create_action_request(source,actor,action_key,normalized,meta,required)

def install()->None:
    if getattr(approvals,"_vp3_scheduling_v059_installed",False):return
    ...
    def approve_request(request_id:str)->dict[str,Any]:
        ...
        try:execution=tools.execute_tool(request["source_app_key"],request["action_key"],request["arguments"],set(),owner=True)
            ...
            with db() as connection:connection.execute("UPDATE action_requests SET status='failed', execution_tool_run_id=?, error=?, executed_at=CURRENT_TIMESTAMP WHERE id=? AND status='executing'",(run_id,str(exc)[:1000],request["id"]))
            raise approvals.ApprovalError(f"Approved scheduling action could not execute: {exc}",exc.status_code) from exc
        with db() as connection:
    ...
    def review_for_app(app_key:str,request_id:str,decision:str)->dict[str,Any]:
        ...
        if existing.get("action_key") in ACTIONS and str(decision).strip().lower()=="approve":raise approvals.ApprovalError("VP3 booking mutations require local HomeServer owner approval.",403)
        return original_review(app_key,request_id,decision)
    approval_federation.review_request_for_app=review_for_app;approvals._vp3_scheduling_v059_installed=True
