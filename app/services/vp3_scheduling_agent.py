...
def install() -> None:
    if getattr(agent_tools, "_vp3_scheduling_v059_installed", False):
        return
    ...
    def model_tool_schemas(
        ...
        *,
        ...
        scope = None if owner or not source_app_key else app_scopes.get_scope_for_source(source_app_key)
        for model_name, tool_key in READS.items():
            item = available.get(tool_key)
            if not item or (scope is not None and not app_scopes.tool_allowed(scope, tool_key)):
                continue
            schemas.append(
                {
                    "type": "function",
    ...
    def execute_model_tool(
        ...
        *,
        ...
        if model_tool_name in READS:
            tool_key = READS[model_tool_name]
            return tools.execute_tool(
                source_app_key,
                tool_key,
                arguments or {},
                set(granted_permissions or set()),
                owner=owner,
            )
        if model_tool_name not in WRITES:
        ...
        if not policy["enabled"] or not policy["allow_write_proposals"]:
            raise agent_tools._deny_unavailable(source_app_key, owner)
        tool_key = WRITES[model_tool_name]
        granted = set(granted_permissions or set())
        available = {
            item["key"]: item
            for item in tools.list_tools(granted, owner=owner)
        ...
        if not agent_tools._scope_allows(source_app_key, tool_key, args, execution):
            raise agent_tools._deny_unavailable(source_app_key, owner)
        result = vp3_scheduling_approvals.create_request(
            source_app_key,
            tool_key,
            args,
            owner=owner,
        ...
        agent_tools._record_policy(
            ...
            reason="VP3 scheduling mutation requires local owner approval before cloud execution.",
        )
        return result

    agent_tools.execute_model_tool = execute_model_tool
    agent_tools._vp3_scheduling_v059_installed = True
