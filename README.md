...
VP3 OS v0.70 adds **Local Automation Rules & Routines**: owner-defined manual/daily/device-state triggers, bounded state conditions, ordered routines, cooldown/rate-limit guardrails, persistent execution audit, and a local scheduler. Rules can only create zero-authority suggestions or ordinary `devices.command` approval requests; they never call provider drivers directly. See [docs/VP3_OS_LOCAL_AUTOMATION_V070.md](docs/VP3_OS_LOCAL_AUTOMATION_V070.md).
