...
- Agent write tools are proposals only by default and use the existing owner approval queue
...
- collection scope, knowledge-kind scope, active-app status, and tool-name scope are rechecked when the owner approves the request
...
- successful mutations immediately reconcile the Knowledge index
- tool/action audit metadata stores lengths and references lengths, not file contents or local filesystem paths
- action requests remain owner-visible, expire under the existing approval lifecycle, and may be denied without side effects

## Execution policy

`files.update` and `files.delete` are write-mode tools. Their inherited paired-app execution policy is `approval_required`. They may be explicitly blocked as `sensitive_high_impact`. File mutations are never made read-only operations.

## Compatibility

Section 9 `files.list` and `files.read` remain unchanged. Section 8 collection semantics remain authoritative, including direct item assignment overriding watched-source defaults.